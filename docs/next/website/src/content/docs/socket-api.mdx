...
selected client. The client opens a local path directly or an SSH endpoint as
`ssh://<target>/<remote-path>`. The response is `type: "client_open_workspace"`
with `opened` and a reason such as `opened`, `no_target_client`,
`unsupported_client`, `endpoint_mismatch`, `workspace_not_found`,
`invalid_path`, `unsupported_opener`, `launch_failed`, or `timed_out`.
