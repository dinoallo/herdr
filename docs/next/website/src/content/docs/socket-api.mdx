...
| Client | `client.window_title.set`, `client.window_title.clear`, `client.open_workspace` |
...
Open a workspace on the foreground client or on the client identified by an invocation token:

```json
{"id":"req_open_workspace","method":"client.open_workspace","params":{"workspace_id":"w1","opener":"zed","target":{"kind":"invocation","invocation_id":"endpoint:boot:7:request"}}}
```

The server resolves the workspace checkout path and forwards the request to the
selected client. The client resolves `opener` against its own `[[openers]]`
config and opens the workspace with the entry's argv template: local endpoints
use `argv`, SSH endpoints use `argv_remote` with the remote path and SSH target.
Herdr core ships no editor definitions. The response is
`type: "client_open_workspace"` with `opened`, a reason such as `opened`,
`no_target_client`, `unsupported_client`, `endpoint_mismatch`,
`workspace_not_found`, `invalid_path`, `unsupported_opener`,
`opener_misconfigured`, `launch_failed`, or `timed_out`, and an optional
`message` with the client-side failure detail.
