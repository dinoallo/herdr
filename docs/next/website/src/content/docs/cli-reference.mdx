...
## Opening a workspace locally

```bash
herdr open-workspace <workspace-id> [--opener NAME] [--invocation-id TOKEN|--foreground]
```

`open-workspace` asks the foreground Herdr client, or the client identified by
an invocation token, to open the workspace in an editor. The server resolves the
workspace directory and the client resolves the opener from its own
`[[openers]]` config, so a local workspace uses its local path while a remote
endpoint passes the opener the remote path plus the SSH target.

`--opener` is optional when exactly one opener is configured. See
[Openers](/docs/configuration/#openers) for the entry format and examples.
