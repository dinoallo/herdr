...
```bash
herdr open-workspace <workspace-id> [--opener zed] [--invocation-id TOKEN|--foreground]
```

`open-workspace` asks the foreground Herdr client, or the client identified by
an invocation token, to open the workspace in an editor. The client resolves
the selected endpoint: local workspaces use their local path, while saved SSH
machines open as `ssh://<target>/<remote-path>`. The only opener currently
implemented is `zed`.
