...
`HERDR_PLUGIN_ENTRYPOINT_ID`. Action commands invoked from a client also receive
`HERDR_PLUGIN_INVOKING_CLIENT_TOKEN`, an opaque token that can be passed to
`client.open_workspace` to target the client that invoked the action.
...
Plugins that need to open a workspace on the user's local machine can call
`client.open_workspace` through `HERDR_BIN_PATH`. The server forwards the
workspace id and opener to the foreground client, or to the client identified
by `HERDR_PLUGIN_INVOKING_CLIENT_TOKEN`; the client owns endpoint and opener
resolution.

Plugins are not required for simple editors. A `type = "open_workspace"`
keybinding calls the same API directly on the client that pressed the key, so
editors that only need an argv template can be configured without installing a
plugin on every machine that hosts a workspace. See
[Openers](/docs/configuration/#openers).
